...
import { Btn, Icon, AgentAvatar, IntegrationTile, INTEGRATIONS } from "../ui";
...
function IntegrationCard({ id, connected, onToggle }) {
  const m = INTEGRATIONS[id],
    d = INT_DETAIL[id];
  return (
    <div
      className="card"
      style={{ borderRadius: 16, padding: 18, display: "flex", flexDirection: "column", gap: 14, borderColor: connected ? "var(--accent-ring)" : "var(--border)" }}
    >
      ...
      <div>
        ...
        <p className="muted" style={{ fontSize: 13, lineHeight: 1.5, marginTop: 5 }}>
          {d.desc}
        </p>
      </div>
      <div className="row between center" style={{ marginTop: "auto" }}>
        {connected ? (
          <span className="row center" style={{ gap: 7, fontSize: 12.5, color: "var(--text-2)", minWidth: 0 }}>
            ...
            <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.account || "Connected"}</span>
          </span>
        ) : (
        ...
        {connected ? (
          <Btn variant="ghost" size="sm" className="btn-danger" onClick={() => onToggle(id)}>
            Disconnect
          </Btn>
        ) : (
          <Btn variant="primary" size="sm" icon="plug" onClick={() => onToggle(id)}>
            Connect
          </Btn>
        )}
      </div>
...
export function Integrations({ app }) {
  ...
  return (
    <PageWrap
      ...
      actions={
        agent && (
          <div className="row center" style={{ gap: 9, padding: "7px 12px 7px 8px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 11 }}>
    ...
    >
      <div
        className="card"
        style={{ borderRadius: 16, padding: "18px 22px", marginBottom: 22, display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(120deg, var(--accent-t), var(--surface))" }}
      >
        <div className="row center" style={{ gap: 14 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: "var(--accent)", color: "#fff", display: "grid", placeItems: "center" }}>
            <Icon name="plug" size={22} />
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: 16 }}>
              {count} of {INT_ORDER.length} tools connected
            </div>
            <div className="muted" style={{ fontSize: 13.5, marginTop: 1 }}>
              Connected tools let your agent act — not just talk.
            </div>
          </div>
        </div>
        <div className="row center">
          {[...connected].slice(0, 5).map((id, i) => (
            <div key={id} style={{ marginLeft: i ? -8 : 0, border: "2px solid var(--surface)", borderRadius: 11 }}>
              <IntegrationTile id={id} size={34} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
        {INT_ORDER.map((id) => (
          <IntegrationCard key={id} id={id} connected={connected.has(id)} onToggle={toggle} />
        ))}
      </div>
      ...
      <div className="row center" style={{ gap: 10, marginTop: 24, padding: "14px 18px", background: "var(--surface-2)", borderRadius: 12 }}>
        ...
        <span className="muted" style={{ fontSize: 13 }}>
          Agent Bora requests the narrowest scopes it needs. You can revoke any connection at any time.
        </span>
      </div>
    </PageWrap>
  );
