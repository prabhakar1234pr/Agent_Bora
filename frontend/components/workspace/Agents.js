...
import { Btn, Icon, AgentAvatar, UserAvatar, Wave, StatusDot, EmptyState, SOURCE_META } from "../ui";
  ...
  return (
    ...
    >
      ...
      <div className="col grow" style={{ gap: 4, padding: "0 10px", overflowY: "auto" }}>
        {app.agents.map((a) => {
          ...
          return (
            <button
              key={a.id}
              onClick={() => app.setCurrentAgentId(a.id)}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 11,
                ...
                background: active ? "var(--accent-t)" : "transparent",
                textAlign: "left",
                transition: "all .14s",
              }}
            >
      ...
      <div style={{ padding: 12, borderTop: "1px solid var(--border)" }}>
        <Btn variant="secondary" block icon="plus" onClick={() => app.nav("createAgent")}>
          New agent
        </Btn>
      </div>
    </div>
...
function agentReply(agent, text) {
  ...
  if (t.includes("action"))
    return "You own 3 open items right now: source-check the investor figures (due May 18 — overdue), assemble the Villa security packet (Jun 3), and draft the Q4 pricing brief (Jun 9). Want me to draft any of them, or push them to Asana?";
  if (t.includes("follow") || t.includes("email"))
    return "Drafted. Subject: “Q3 locked — here’s where each bet stands.” It recaps the three bets, the billing slip and why, and lists the four owners with dates. Want me to open it in Gmail or tweak the tone first?";
  if (t.includes("roadmap") || t.includes("chang"))
    return "Since last week: billing v2 moved to mid-July for the SOC 2 window, mobile beta is now sequenced after the editor GA, and the pricing experiment was parked in Q4. Two action items from the lock are still unowned.";
  if (t.includes("summ") || t.includes("last meeting"))
    return "Q3 Roadmap Lock (48m, 4 people): the team confirmed three bets — editor, billing v2, mobile beta. Billing slipped two weeks for SOC 2. I flagged the editor/mobile resource clash and we sequenced them. 4 action items captured with owners.";
...
function AgentChat({ app, agent }) {
  ...
  useEffect(() => {
    setMsgs(app.agentChats[agent.id] || []);
          ...
          <div>
            <div className="row center" style={{ gap: 9 }}>
              ...
              <span className={"badge " + (agent.voice === "proactive" ? "badge-accent" : "badge-muted")}>
                <Icon name={agent.voice === "proactive" ? "mic" : "eye"} size={12} />
                {agent.voice === "proactive" ? "Proactive" : "Passive"}
              </span>
            </div>
            <div className="row center" style={{ gap: 8, marginTop: 2 }}>
              ...
              <span className="muted" style={{ fontSize: 12.5 }}>
                {agent.sources.length} sources · {agent.meetings} meetings
              </span>
            </div>
          </div>
        </div>
        <div className="row center" style={{ gap: 9 }}>
          <Btn variant="secondary" icon="plug" onClick={() => app.nav("integrations")}>
            Integrations
          </Btn>
          <Btn variant="primary" icon="mic" onClick={() => app.nav("newMeeting")}>
            Call to meeting
          </Btn>
          <button className="btn btn-ghost btn-icon" onClick={() => app.nav("createAgent", { edit: agent.id })}>
        ...
        <div className="col" style={{ gap: 16, maxWidth: 760, margin: "0 auto" }}>
          <div className="row center" style={{ gap: 10, alignSelf: "center", padding: "8px 14px", background: "var(--surface-2)", borderRadius: 999, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 12.5, color: "var(--text-2)" }}>
              {agent.name} has context from {agent.sources.length} sources and {agent.integrations.length} connected tools
            </span>
          </div>
          {msgs.map((m, i) => (
            <ChatBubble key={i} m={m} agent={agent} />
          ))}
          {typing && (
      ...
      <div style={{ padding: "14px 24px 20px", background: "var(--surface)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          >
            ...
            />
            <button className="btn btn-primary btn-icon" onClick={() => send()} disabled={!draft.trim()} style={{ opacity: draft.trim() ? 1 : 0.5 }}>
      ...
      <PageWrap title="Agents" subtitle="Your AI teammates live here.">
        <div className="card" style={{ borderRadius: 20, marginTop: 8 }}>
          <EmptyState
            ...
            action={
              <Btn variant="primary" size="lg" icon="plus" onClick={() => app.nav("createAgent")}>
                Create agent
              </Btn>
            }
          />
...
export function CreateAgent({ app }) {
  ...
  const create = () => {
    ...
    app.createAgent({
      ...
      persona: persona.trim() || "A helpful meeting agent.",
      voice,
  ...
  return (
    <div className="app-scroll fade-in">
      <div style={{ maxWidth: 840, margin: "0 auto", padding: "28px 28px 80px" }}>
        <button className="btn btn-ghost btn-sm" onClick={() => app.nav("agents")} style={{ marginBottom: 18, paddingLeft: 8 }}>
        ...
        <div className="row center" style={{ gap: 18, marginBottom: 30 }}>
          ...
          <div>
            ...
            <p className="muted" style={{ fontSize: 15, marginTop: 3 }}>
              Give it an identity, a voice, and all the context you can.
            </p>
          </div>
        </div>
        ...
        <div className="card card-pad" style={{ borderRadius: 18, marginBottom: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 18 }}>
            <div className="field">
              ...
              <input className="input" placeholder="e.g. Atlas" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
            </div>
            <div className="field">
              <span className="label">One-line role</span>
              <input className="input" placeholder="e.g. Product & roadmap copilot" value={tagline} onChange={(e) => setTagline(e.target.value)} />
            </div>
          </div>
          <div className="field" style={{ marginTop: 16 }}>
            ...
            <textarea
              className="textarea"
              rows={3}
              placeholder="How should it behave? When should it speak up? e.g. “Flag scope creep. Track owners and dates. Stay concise.”"
              value={persona}
              ...
              return (
                ...
                >
                  <div className="row between center" style={{ marginBottom: 10 }}>
                    ...
                    >
                      {on && <div style={{ width: 10, height: 10, borderRadius: "50%", background: "var(--accent)" }} />}
                    </div>
                  </div>
        ...
        <div className="card card-pad" style={{ borderRadius: 18, marginBottom: 24 }}>
          ...
          <div className="row wrap" style={{ gap: 8, margin: "18px 0 14px" }}>
            {SOURCE_TYPES.map((s) => (
              <button
                key={s.type}
                className={"chip" + (activeType === s.type ? " is-on" : "")}
                onClick={() => {
          ...
          <div className="row center" style={{ gap: 10 }}>
            {activeType === "doc" ? (
              ...
              >
                ...
                <span style={{ fontSize: 13.5 }}>Drag a file here, or click to browse</span>
              </div>
            ) : (
              <input
                className="input"
            ...
            <Btn variant="primary" icon="plus" onClick={addSource}>
              Add
            </Btn>
          </div>
              ...
              {sources.map((s, i) => {
                ...
                return (
                  <div key={i} className="row center between" style={{ padding: "10px 12px", borderRadius: 11, background: "var(--surface-2)" }}>
                    ...
                    <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setSources(sources.filter((_, j) => j !== i))}>
                      <Icon name="x" size={15} />
                    </button>
                  </div>
                );
          ...
          {sources.length === 0 && (
            <p className="faint" style={{ fontSize: 13, marginTop: 14, textAlign: "center" }}>
              No sources yet — add a few above. You can always add more later.
            </p>
          )}
        </div>
        ...
        >
          <span className="muted" style={{ fontSize: 13 }}>
            {sources.length} source{sources.length !== 1 ? "s" : ""} · {voice} mode
          </span>
          <div className="row center" style={{ gap: 10 }}>
            <Btn variant="ghost" onClick={() => app.nav("agents")}>
              Cancel
            </Btn>
            <Btn variant="primary" size="lg" icon="sparkle" disabled={!canCreate} onClick={create} style={{ opacity: canCreate ? 1 : 0.5 }}>
              Create agent
            </Btn>
          </div>
        </div>
