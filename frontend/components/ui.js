/* ui.js — shared design-system primitives for Agent Bora
   (ported from the prototype's ui.jsx; window globals replaced with ES exports) */
import { BrandLogo, BRAND } from "./BrandLogos";

/* ---------------- Icons (minimal line set) ---------------- */
export const ICONS = {
  home: "M3 10.5 12 3l9 7.5M5 9.5V20a1 1 0 0 0 1 1h3v-6h6v6h3a1 1 0 0 0 1-1V9.5",
  agent:
    "M12 3v3M9 9h6a3 3 0 0 1 3 3v4a3 3 0 0 1-3 3H9a3 3 0 0 1-3-3v-4a3 3 0 0 1 3-3ZM9.5 14h.01M14.5 14h.01M4 13H3M21 13h-1",
  ...
  github:
    "M9 19c-4 1.5-4-2-6-2.5m12 5v-3.5a3 3 0 0 0-.8-2.3c2.7-.3 5.5-1.3 5.5-6a4.6 4.6 0 0 0-1.3-3.2 4.3 4.3 0 0 0-.1-3.2s-1-.3-3.4 1.3a11.7 11.7 0 0 0-6 0C6.9 3.7 5.9 4 5.9 4a4.3 4.3 0 0 0-.1 3.2A4.6 4.6 0 0 0 4.5 10.4c0 4.6 2.8 5.7 5.5 6a3 3 0 0 0-.8 2.2V22",
  ...
  chevD: "M6 9l6 6 6-6",
  sparkle:
    "M12 3l1.6 4.8L18 9.4l-4.4 1.6L12 16l-1.6-5L6 9.4l4.4-1.6L12 3ZM19 14l.8 2.2L22 17l-2.2.8L19 20l-.8-2.2L16 17l2.2-.8L19 14Z",
  ...
  users:
    "M16 20v-1.5a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4V20M9 11a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7ZM22 20v-1.5a4 4 0 0 0-3-3.8M16 4.3a3.5 3.5 0 0 1 0 6.8",
  ...
  brain:
    "M8.5 4a3 3 0 0 0-3 3 3 3 0 0 0-1 5.8A3 3 0 0 0 8.5 18a2.5 2.5 0 0 0 3.5-2.3V6a2.5 2.5 0 0 0-3.5-2ZM15.5 4a3 3 0 0 1 3 3 3 3 0 0 1 1 5.8A3 3 0 0 1 15.5 18a2.5 2.5 0 0 1-3.5-2.3",
  refresh: "M3 12a9 9 0 0 1 15-6.7L21 8M21 3v5h-5M21 12a9 9 0 0 1-15 6.7L3 16M3 21v-5h5",
  rocket:
    "M5 15c-1.5 1.3-2 5-2 5s3.7-.5 5-2M9 11a13 13 0 0 1 8-8c2 0 3 1 3 3a13 13 0 0 1-8 8l-3-3ZM9 11l-4 1 .5 2.5L7 15l1 .5 1.5 1.5 1-4M14 8.5a1.5 1.5 0 1 0 0 .01",
...
/* ---------------- Logo ---------------- */
export function Logo({ size = 26, withWord = true, color }) {
  const s = size;
  return (
    <div className="row center" style={{ gap: 10 }}>
      <div
        style={{
          ...
          position: "relative",
          background:
            "linear-gradient(145deg, var(--accent) 0%, color-mix(in oklab, var(--accent) 55%, #0ea5e9) 100%)",
          boxShadow: "inset 0 1px 1px rgba(255,255,255,.35), var(--shadow-sm)",
          display: "grid",
          placeItems: "center",
          flex: "none",
        }}
...
/* ---------------- Button ---------------- */
export function Btn({ children, variant = "secondary", size = "", icon, iconRight, block, className = "", ...rest }) {
...
export function AgentAvatar({ name = "Agent", size = 40, square = false, listening = false, glyph = true }) {
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flex: "none" }}>
      ...
      >
        {glyph ? (
          <Icon name="agent" size={size * 0.56} stroke={1.9} style={{ color: "#fff", opacity: 0.96 }} />
        ) : (
          initials(name)
        )}
      </div>
...
export function Wave({ size = 18, color = "var(--accent-d)", bars = 5, big = false }) {
  return (
...
/* ---------------- User avatar ---------------- */
export function UserAvatar({ name = "You", size = 34 }) {
  return (
    <div
      className="avatar"
      style={{
        ...
        height: size,
        fontSize: size * 0.4,
        background: "linear-gradient(140deg,#475569,#0f172a)",
      }}
    >
      {initials(name)}
    </div>
...
/* ---------------- Status dot ---------------- */
export function StatusDot({ color = "var(--accent)", label, pulse = true }) {
  return (
...
export function IntegrationTile({ id, size = 40, radius }) {
  if (BRAND[id]) return <BrandLogo id={id} size={size} radius={radius} />;
  ...
  return (
    <div
      style={{
        ...
        placeItems: "center",
        flex: "none",
        border: "1px solid color-mix(in oklab, " + m.fg + " 12%, transparent)",
      }}
    >
...
export function EmptyState({ icon, title, body, action }) {
  return (
    <div className="col center" style={{ textAlign: "center", padding: "56px 24px", gap: 6 }}>
      ...
      <p className="muted" style={{ maxWidth: 360, fontSize: 14.5, lineHeight: 1.6 }}>
        {body}
      </p>
...
/* ---------------- Tag pill ---------------- */
export function Pill({ children, tone = "muted", icon }) {
  const cls =
    tone === "accent"
      ? "badge badge-accent"
      : tone === "amber"
